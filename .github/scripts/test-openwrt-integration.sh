...
sh -n package/nginx-telego/files/usr/libexec/nginx-telego-platform
...
bash .github/tests/nginx-telego-platform.sh
 ...
 * 2
assert router_verify.count('/tmp/nginx-telego-platform-preflight.' + pid_marker) == 3
assert router_verify.count('/tmp/nginx-telego-tls-probe.' + pid_marker + '.out') == 1
assert router_verify.count('/tmp/nginx-telego-unknown-sni.' + pid_marker + '.out') == 1
assert '/tmp/nginx-telego-tls-probe.$.out' not in router_verify
assert '/tmp/nginx-telego-unknown-sni.$.out' not in router_verify
print('Direct HTTPS hardware verifier temp-file contract passed')
PY
python3 .github/scripts/check-doc-links.py
python3 -m py_compile .github/scripts/apply-upstream-patches.py .github/scripts/check-doc-links.py
