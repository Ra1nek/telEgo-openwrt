...
exercise_nginx_reconciliation() {
  # Simulate the two historical paths from pre-final P7 packages. The clean old
  # core matches the canonical package bytes and the combined generated file has
  # the historical ownership marker, so both must migrate without console cleanup.
  cp /usr/share/nginx-telego/templates/20-telego-core.conf /etc/nginx/conf.d/telego.conf
  printf '%s\n# smoke legacy generated state\n' \
    '# Generated by /usr/libexec/nginx-telego-render from /etc/config/nginx_telego.' \
    >/etc/nginx/conf.d/zz-telego-managed.conf
  /usr/libexec/nginx-telego-reconcile apply --no-reload
  test ! -e /etc/nginx/conf.d/telego.conf
  ...
  /usr/libexec/nginx-telego-files status >/tmp/nginx-telego-status-after-migration.tsv
  grep -q '^/etc/nginx/conf.d/20-telego-core.conf.*core.*package.*required.*ok' /tmp/nginx-telego-status-after-migration.tsv
}
