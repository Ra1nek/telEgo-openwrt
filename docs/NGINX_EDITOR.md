# P9 — ограниченный редактор Nginx

[**Русский**](NGINX_EDITOR.md) · [English](NGINX_EDITOR_EN.md)

P9 добавляет в существующую страницу **Службы → telEgo → Файлы Nginx** ограниченный редактор для уже существующих administrator-owned/foreign файлов `*.conf`.

P9 не меняет модель владения P6/P7/P8: редактирование файла не делает его telEgo-owned и не меняет `ownership.tsv`.
...
Создание/rename новых `.conf` намеренно не входят в первый P9 scope.
